...
threshold <- median(train.data$e3_bw, na.rm = TRUE)  # Use the median as the threshold

# Convert the outcome variable to a factor with "High" and "Low"
train.data$e3_bw <- ifelse(train.data$e3_bw > threshold, "High", "Low")
test.data$e3_bw <- ifelse(test.data$e3_bw > threshold, "High", "Low")

# Ensure the outcome variable is a factor
train.data$e3_bw <- as.factor(train.data$e3_bw)
test.data$e3_bw <- as.factor(test.data$e3_bw)
...
# Model coefficients
coef(logreg.model$finalModel)

# Predictions on the test set
predictions <- predict(logreg.model, test.data)

# Calculate accuracy
confusionMatrix(predictions, test.data$e3_bw)
```
